package sample.controller;// William Ames Audio Utilities
// Spring 2010
...
public class Audio {
	...
	private byte[] convertToBytes(boolean side) {
		checkMaximum();
		byte[] audioBytes = new byte[left.length * 2 * 2];
		int bytePos = 0;
		if (left.length != right.length) {
	...
	public void splitAndSave(String fileName) {
		for (int i = 0; i < 2; i++) {
			File fileOut = new File(fileName + i + ".wav");
			byte[] bytes = convertToBytes(i == 0);
			ByteArrayInputStream byteStream = new ByteArrayInputStream(bytes);
			AudioFormat format = desiredFormat;
			AudioInputStream stream = new AudioInputStream(byteStream, format,
					bytes.length / 2 / 2);
